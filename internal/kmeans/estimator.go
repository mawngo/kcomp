...
// NewEstimator Implementation of cluster number estimator using gap statistic
// ("Estimating the number of clusters in a data set via the gap statistic", Tibshirani et al.) with k-means++ as
// clustering algorithm
func NewEstimator(iterations, clusters int, distance DistanceFunc) *Estimator {
	if iterations < 1 {
...
func (c *Estimator) Estimate(data [][]float64) int {
	if len(data) == 0 {
		panic(ErrEmptySet)
	}
...
// private
func (c *Estimator) learn(data [][]float64) {
	c.d = data
...
func (c *Estimator) initializeMeansWithData() {
...
func (c *Estimator) wk(data [][]float64, centroids [][]float64, mapping []int) float64 {
	var (
...
func (c *Estimator) buildRandomizedSet(size int, bounds []*[2]float64) [][]float64 {
	var (
		l = len(bounds)
		r = make([][]float64, size)
	)
